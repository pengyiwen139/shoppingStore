...
<body>
	...
	<div class="message">
		...
		<c:forEach items="${list_address}" var="address">
				${address.yh_dz}
			<div class="msg_addr">
				<span class="msg_left"> ${address.shjr}<br> 联系电话：${address.lxfsh} </span> <span
					class="msg_right"> <input type="radio"
					onclick="select_address(${address.id},${address.yh_dz})"
					name="a" />${address.yh_dz}
		...
		<c:forEach items="${order.list_flow}" var="flow" varStatus="index">
			<c:if test="${not empty flow }">
				<div style="border: red 1px solid; margin-top: 10px;">
					配送方式:${flow.psfsh}<br>配送状态：${flow.mqdd}<br>
		...
		<div class="msg_sub">
			<form action="save_order.do" method="post">
				...
				<div class="msg_sub_adds" id="check_order_div">寄送至 ： 北京市 昌平区
					北七家镇 尚硅谷IT教育 &nbsp;${order.shhr}（收） 185****1222</div>
